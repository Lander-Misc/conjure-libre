...
local function source_file_path(opts)
  if (opts["file-path"] and ("" ~= opts["file-path"])) then
...
M["prep-code"] = function(opts)
  local s = opts.code
  local python_expr = M["str-is-python-expr?"](s)
  if python_expr then
    return (get_file_str(opts) .. s .. "\n")
  else
    return get_exec_str(opts)
  end
end
